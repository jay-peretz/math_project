...
angular.module('mathSkills') 
	.config(['parserProvider', function (parserProvider) {
        parserProvider.register('exposelect', {
            directiveTemplate: '<ms-exponent-select expected={{expected}} label={{label}}></ms-exponent-select>'
        });
    }])
  .directive('msExponentSelect', [
	'parser',
    'directiveUtils',
    'problemData',
    '$timeout',
    function (parser, directiveUtils, problemData, $timeout) {
        return {
            ...
            template: '<div class="control-group {{class}}"><sup><select style="width:{{width}}; margin:0;" ng-model="answer" ng-options="value for value in optionsarray"> <option value="">-- choose --</option></select></sup></label></div>'
        };
    }]);