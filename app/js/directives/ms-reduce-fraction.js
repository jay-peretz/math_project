		...
		controller: ['$scope',function($scope) {
			...
			$scope.$watch('expected',function() {
				if ($scope.expected) {
					...
					$scope.answerexpression = simplified($scope.args[0],$scope.args[1]) ? no : yes ;
					$scope.instructions = "Can the fraction be simplified?";
					$scope.labelexpression = "simplified";
				}
				...
				switch (data.label) {
					case "simplified" : 
						if (data.result === "correct") {
							...
							$scope.labelexpression = "factor";
							$scope.answerexpression = '\\input{['+commonFactors($scope.args[0],$scope.args[1]).join(',')+']}';
							$scope.$apply();
							console.log($scope.answerexpression);
						} else {
